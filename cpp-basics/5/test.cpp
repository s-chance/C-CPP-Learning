...
int main() {
  ...
  int num, i;
  cin >> num;  // 5
  myVector<myString> aStrSet(num), bStrSet(num - 1),
      cStrSet;  // 构建3个字符串（向量）集合
  ...
  cout << "字符串集a的构成为：" << endl;
  for (i = 0; i < aStrSet.get_size(); i++) {
    cout << aStrSet[i];  // 增加myString的输出流运算支持，将字符串集合a
                         // 输出到屏幕上
  ...
  for (i = 0; i < cStrSet.get_size();
       i++) {  // 将字符串集合a和b的差的结果输出到屏幕上
    cout << cStrSet[i];
  }  // a-rainy-day.
  cout << endl;

  cStrSet.set_size(5);
  for (i = 0; i < 5; i++) cStrSet[i] = str[i];
  ...
  astr++;  // 定义myString的增量操作，将串内的每个字母循环调整为其后面的字母，如abc
           // 调整为bcd；其他符号保持不变。
  cout << astr;  // Upebz-jt-b-tvmmz-ebz.
  cout << endl;

  cout << strlen(
      astr);  // 定义myString的类型转换操作，使其可以使用strlen, 输出 21
